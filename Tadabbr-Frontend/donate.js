...
amountSlider.addEventListener('input', function() {
    ...
    selectedAmountDiv.textContent = `${amount} ريال`;
...
async function copyAccountDetails(method) {
    let textToCopy = '';
    if (method === 'bank') {
        textToCopy = `اسم البنك: بنك الراجحي\nرقم الحساب: 1234567890\nرقم الآيبان: SA121230000001234567890`;
    } else if (method === 'paypal') {
         showNotification('سيتم فتح صفحة باي بال للتبرع.', true);
         
         window.open('https://www.paypal.me/yourpaypal', '_blank');
         return;
    }
    try {
        await navigator.clipboard.writeText(textToCopy);
        showNotification('تم نسخ التفاصيل!', false);
    } catch (err) {
